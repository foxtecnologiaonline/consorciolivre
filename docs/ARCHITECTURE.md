...
- **Pagamentos/escrow**: **Pagar.me** — decisão única, não mais em aberto. Split nativo via `recipient_id`, mas aplicado só na **liberação**, não na cobrança — é o que garante o escrow de verdade:
  1. Com o KYC aprovado, o vendedor cadastra a própria conta bancária (`/painel/dados-bancarios`, tabela `contas_bancarias`) e o backend cria o recebedor no Pagar.me (`POST /core/v5/recipients`), gravando o id em `profiles.pagarme_recipient_id` — coluna protegida por trigger contra escrita direta do client (só um Server Action com `service_role` grava, depois que o Pagar.me já confirmou a criação). Sem KYC aprovado **e** `pagarme_recipient_id`, publicar anúncio é bloqueado (`lib/vendedor/elegibilidade.ts`).
  2. Na cobrança (`POST /core/v5/orders`), o valor cheio da transação fica **100% com a conta da própria plataforma** — sem `split` nenhum nesse momento. É isso que mantém o dinheiro em escrow: se o split fosse feito já na cobrança, o Pagar.me poderia repassar o valor ao `recipient_id` do vendedor antes mesmo da transferência de titularidade ser confirmada (o `transfer_interval` do recebedor é automático por padrão), quebrando a garantia de escrow do produto.
  3. Só quando o comprador confirma a transferência de titularidade (`transacoes.status = 'concluida'`) a plataforma dispara uma transferência explícita (`POST /core/v5/recipients/{id}/withdrawals` ou equivalente) do valor líquido (`valor_acordado - comissao_valor`) para o `recipient_id` do vendedor — aí sim o dinheiro sai da conta da plataforma. Ver §8 item 6.
...
3. ~~Integração de cobrança real (PIX)~~ — Server Action cria um pedido no Pagar.me (`POST /core/v5/orders`, método `pix`, **sem split** — 100% para a conta da plataforma, ver §5.1) para o `valor_acordado` da transação, grava em `pagamentos` (`gateway = 'pagarme'`, `gateway_referencia` = id do pedido) e mostra o QR code/copia-e-cola ao comprador. **Concluído** — cartão e boleto ficam fora deste item (fase 2.1) — PIX cobre o caso de uso dominante sem exigir tokenização de cartão no front; payload não validado contra sandbox real (mesma ressalva do item 2).
...
6. ~~Fluxo de disputa guiado~~ — a Server Action de staff (`painel/admin/disputas`) agora chama de verdade o Pagar.me: "liberar ao vendedor" dispara `liberarEscrow` (mesma transferência do item 5), "estornar comprador" cancela a charge (`POST /core/v5/charges/{id}/cancel`) via `estornarEscrow`, "dividir" aplica um percentual definido por staff (`lib/pagamentos/divisao.ts`, testado) — parte transferida ao vendedor, parte estornada ao comprador, via `dividirEscrow`, todas em `lib/pagamentos/escrow.ts`. **Concluído**. Novo status terminal `dividida` (`transacoes`/`pagamentos`, migration `0009_divisao_disputa.sql`) — decisão de produto tomada sem confirmação explícita do dono do produto: divisão de valor **não** pressupõe nada sobre a posse da cota (fica registrado na observação do evento; staff decide separadamente, pelo painel de anúncios, se reabre o anúncio). Revisar esse default se o entendimento de negócio for outro.
