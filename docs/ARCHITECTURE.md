...
3. Integração de cobrança real (PIX): Server Action cria um pedido no Pagar.me (`POST /core/v5/orders`, método `pix`, **sem split** — 100% para a conta da plataforma, ver §5.1) para o `valor_acordado` da transação, grava em `pagamentos` (`gateway = 'pagarme'`, `gateway_referencia` = id do pedido) e mostra o QR code/copia-e-cola ao comprador. Cartão e boleto ficam fora deste item (fase 2.1) — PIX cobre o caso de uso dominante sem exigir tokenização de cartão no front.
4. Webhook do Pagar.me (`POST /api/webhooks/pagarme`, Route Handler com `service_role`): idempotente via `idx_pagamentos_gateway_ref` já existente, atualiza `pagamentos.status` e dispara transição de `transacoes.status` (`aguardando_pagamento` → `pagamento_em_escrow`), com evento em `transacao_eventos`. Substitui o botão manual "Já paguei" do comprador — a confirmação de pagamento passa a ser só o gateway, nunca autodeclarada.
5. Liberação de escrow automatizada: quando o comprador confirma a transferência de titularidade, além de `transacoes.status = 'concluida'`, dispara uma transferência explícita do valor líquido (`valor_acordado - comissao_valor`) da conta da plataforma para o `recipient_id` do vendedor no Pagar.me (só agora o dinheiro sai da plataforma) e atualiza `pagamentos.status = 'liberado_vendedor'`.
